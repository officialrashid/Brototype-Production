...
async function scheduleInteractor() {
  ...
  if (
    ...
    studentsData.length &&
    coordinatorsData.length
  ) {
    console.log(
      "scheduler interactor calleddd",
      studentsData,
      coordinatorsData
    );

    savedReviewData = await reviewController.OncreateReviewData(
      coordinatorsData,
      studentsData
    );
    studentsData = undefined;
    ...
    if (savedReviewData) {
      ...
      const scheduledData = await reviewController.OnScheduleReview(
        id,
        students,
        coordinators
      );
