...
import { getAllAdvisors, getAllChatReviewers, getAllChatStudents, getAllChatSuperleads, getAllStudents, getAllSuperleads, getChatReviewers, getGroupMembersDetails, getGroupMessages } from "../../../utils/methods/get";
...
const GroupInformationModal = ({ isVisible, onClose, changeModalStatus, groupId, groupDetails }: { isVisible: boolean; onClose: () => void; changeModalStatus: () => void; groupId: string; groupDetails: any; }) => {
  ...
  useEffect(() => {
    const fetchGroupChatProfiles = async () => {
      try {
        const students = await getAllChatStudents();
        const superleads = await getAllChatSuperleads();
        const advisors = await getAllAdvisors();
        const reviewers = await getChatReviewers()
        console.log(advisors, "kkkkhhh advisors");
        console.log(reviewers, "kkkkrrrhhh reviewers");
        console.log(superleads, "kkkkrrrhhh superleads");
        console.log(students, "kkkkrrrhhh students");
        if (students?.response?.status === true && superleads?.status === true) {
          const combinedResponses = [...superleads.response, ...students.response.response, ...advisors.response, ...reviewers.response];
  ...
  const handleCheckboxChange = (chatDetails: any) => {
    ...
    if (index === -1) {
      ...
      setParticipants(prevState => [...prevState, chatDetails.studentId || chatDetails._id]);
    } else {
      ...
      const updatedParticipants = participants.filter(id =>
        id !== chatDetails.studentId && id !== chatDetails._id
      );
                        ...
                        <table className="w-full text-sm text- text-gray-500 dark:text-gray-400">
                          ...
                          <tbody>
                            ...
                            {chatParticipantsDetails
                              .filter(chatDetails => !members.some(member => member.chaterId === chatDetails.studentId || member.chaterId === chatDetails._id))
                              .map((chatDetails, index) => (
                                <tr key={index} className="bg-white dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                  <td className="flex items-center px-6 py-4 whitespace-nowrap">
                                    {chatDetails.profileUrl ? (
                                      <img className="w-8 h-8 rounded-full object-cover" src={chatDetails.profileUrl} alt={chatDetails.firstName} />
                                    ) : chatDetails.imageUrl ? (
                                      <img className="w-8 h-8 rounded-full object-cover" src={chatDetails.imageUrl} alt={chatDetails.firstName} />
                                    ) : (
                                      <img className="w-8 h-8 rounded-full object-cover" src="/defaultPhoto.png" alt={chatDetails.firstName} />
                                    )}

                                    <div className="ps-3">
                                      <div className="text-sm font-roboto">{chatDetails.name} {chatDetails.firstName} {chatDetails.lastName}</div>
                                    </div>
                                  </td>
                            ...
                            {members.map((member, index) => (
                              <React.Fragment key={index}>
                                ...
                                <tr key="" className="bg-white  dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                  <td className="flex items-center px-6 py-4 whitespace-nowrap">
                                    {member?.imageUrl ? (
                                      <img className="w-8 h-8 rounded-full object-cover" src={member?.imageUrl} />
                                    ) : (
                                      <img className="w-8 h-8 rounded-full object-cover" src="/defaultPhoto.png" />
                                    )}
